...
async function compileAndRespondWithProject(
  parsedInput: z.infer<typeof projectVideoSchema>,
  sessionId: string,
...
const projectVideoSchema = z.object({
  ...
  compositionId: z.string().optional().default("Main").describe("Composition identifier (default: Main)"),
  width: z.number().optional().default(1920).describe("Fallback composition width in pixels"),
  height: z.number().optional().default(1080).describe("Fallback composition height in pixels"),
  fps: z.number().optional().default(30).describe("Fallback frames per second"),
  durationInFrames: z.number().optional().default(150).describe("Fallback total duration in frames"),
...
server.tool(
  ...
  async (rawParams: z.infer<typeof updateVideoSchema>, ctx) => {
    ...
    const previousProject = sessionProjects.get(sessionId) ?? null;
